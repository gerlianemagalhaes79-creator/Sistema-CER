...
export const PatientSurveyPage = ({ availableSectors = [] }: PatientSurveyPageProps) => {
  ...
  return (
    <div className="min-h-screen bg-[#F0FDF4] text-gray-800 flex flex-col justify-between" id="id_patient_survey_root">
      ...
      <header className="py-4 px-4 md:px-8 max-w-4xl mx-auto w-full">
        <div className="bg-white rounded-[2rem] border border-emerald-100 shadow-sm p-5 sm:p-6 space-y-4">
          
          {/* Top Section: Clinic Logo or Names with discrete speech bubble badge above */}
          <div className="text-center sm:text-left flex flex-col items-center sm:items-start text-emerald-950">
            
            {/* Discreet Speech Bubble (Balãozinho discreto) */}
            <div className="relative inline-flex items-center gap-1 bg-[#01402E] text-[#F0FDF4] text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full shadow-sm mb-3">
              <span>Pesquisa Ativa</span>
              {/* Little speech bubble pin */}
              <div className="absolute top-full left-1/2 sm:left-6 -translate-x-1/2 sm:translate-x-0 w-2 h-2 bg-[#01402E] rotate-45 -mt-1" />
            </div>

            <div className="w-full">
              {loadedLogos.pesquisa ? (
                <div className="flex flex-col items-center sm:items-start gap-2.5">
                  <img 
                    src={loadedLogos.pesquisa} 
                    alt="Logo Policlínica" 
                    className="h-32 sm:h-44 md:h-52 w-auto max-w-full object-contain rounded-xl" 
                    referrerPolicy="no-referrer" 
                  />
                  <p className="text-xs sm:text-sm text-slate-500 font-extrabold uppercase tracking-wide text-center sm:text-left max-w-xs sm:max-w-md leading-relaxed">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs sm:text-sm text-slate-500 font-extrabold uppercase tracking-wide">
                    Pesquisa de Satisfação dos Pacientes
                  </p>
                </div>
              )}
          ...
          <hr className="border-t border-slate-100/70" />

          {/* Step Indicator Row */}
          {step <= 3 && (
            <div className="flex items-center justify-between py-0.5">
              <div className="flex items-center gap-2 text-[#01402E]">
              ...
              <div className="flex gap-1.5">
                <div className={`w-2 h-2 rounded-full ${step >= 1 ? 'bg-[#01402E]' : 'bg-emerald-100'}`} />
                <div className={`w-2 h-2 rounded-full ${step >= 2 ? 'bg-[#01402E]' : 'bg-emerald-100'}`} />
                <div className={`w-2 h-2 rounded-full ${step >= 3 ? 'bg-[#01402E]' : 'bg-emerald-100'}`} />
              </div>
            </div>
          )}
